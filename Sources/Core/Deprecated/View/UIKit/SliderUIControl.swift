...
@available(*, deprecated, message: "Use SparkUISlider instead")
public final class SliderUIControl<V>: UIControl where V: BinaryFloatingPoint, V.Stride: BinaryFloatingPoint {

    private let viewModel: SingleSliderViewModelDeprecated<V>
    ...
    // MARK: - Subviews
    private let indicatorView = UIView(frame: CGRect(origin: .zero, size: CGSize(width: 0, height: SliderConstantsDeprecated.barHeight)))
    private let trackView = UIView(frame: CGRect(origin: .zero, size: CGSize(width: 0, height: SliderConstantsDeprecated.barHeight)))
    private let handle: SliderHandleUIControlDeprecated

    init(viewModel: SingleSliderViewModelDeprecated<V>) {
        self.viewModel = viewModel
        self.handle = SliderHandleUIControlDeprecated(
            viewModel: .init(color: viewModel.handleColor,
                             activeIndicatorColor: viewModel.handleActiveIndicatorColor)
        )
        super.init(frame: .init(origin: .zero, size: .init(width: 0, height: SliderConstantsDeprecated.handleSize.height)))
        self.alpha = self.viewModel.dim
        self.setupBar()
        self.subscribeToViewModel()
        self.translatesAutoresizingMaskIntoConstraints = false
        let defaultHeightConstraint = self.heightAnchor.constraint(equalToConstant: SliderConstantsDeprecated.handleSize.height)
    ...
    public override func layoutSubviews() {
        ...
        if self.range.lowerBound < self.range.upperBound {
            ...
            self.handle.center.x = (self.frame.width - SliderConstantsDeprecated.handleSize.width) * CGFloat(value) + SliderConstantsDeprecated.handleSize.width / 2
        } else {
            self.handle.center.x = SliderConstantsDeprecated.handleSize.width / 2
        }
    ...
    private func moveHandle(to: CGFloat) {
        let absoluteX = max(SliderConstantsDeprecated.handleSize.width / 2, min(to, self.frame.width - SliderConstantsDeprecated.handleSize.width / 2))
        let relativeX = (absoluteX - SliderConstantsDeprecated.handleSize.width / 2) / (self.frame.width - SliderConstantsDeprecated.handleSize.width)
